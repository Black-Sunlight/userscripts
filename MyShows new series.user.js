	...
	$(function(){
		...
		$('tr[data-id]').each(function(i){
			...
			$('#loader2'+i).on('click',function(){
				...
				GM.xmlHttpRequest({
								method: "GET",
								url: "http://"+domaintocheck+"/serial.php?id=612",
								headers: {
								...
								onload: function(response) {
									...
									var el=doc.getElementById('tabs-container');
									alert(el.innerHTML);
									for (var k = 0; k < el.getElementsByTagName('a').length;k++){
										if(el[k].innerHTML.search('HD1080p')!=-1){
										...
										GM.xmlHttpRequest({
											...
											onload: function(response) {
												var docr = new DOMParser().parseFromString(response.responseText, "text/html");
												var lnk=docr.getElementsByClassName('linkdownload')[0].getAttribute("href");
												alert(lnk);
										}
										})
										}
