...
$(function(){
	...
	$('div.title__primary').first().before('<style>.seasonBlockBody a[href*="myshows"]{vertical-align: -webkit-baseline-middle;}</style><div id="newseries" class="seasonBlockBody" style="display:inline-block;width: 730px;"><button id="loadnew">Загрузить список выхода серий</button></div>');
	...
	$('#loadnew').on('click',function(){
		$.get("https://myshows.me/my/",function(data){
			$("#newseries").html($("div.WatchSoon.MyHome-shows",data).html());
		});
	});
	$('div[id^="s"]').each(function(i){
		var that=$(this);
		var thatnameblock=that.find('.Unwatched-showTitle');
		...
		$('#loader2'+i).on('click',function(){
			...
			var name=that.find('.Unwatched-showTitle').find('div').text().replace(/\s{2,}/g, ' ');
			name=name.trim();
			...
			var fullname=name.trim()+' '+season.trim();
