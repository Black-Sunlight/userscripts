// ==UserScript==
// @name         MyShows & Coldfilm Auto Checker float
// @namespace    https://github.com/yourname
// @version      5.4
// @description  Проверка серий. Использует beta-парсинг с GM.xmlHttpRequest. UI + fallback + автоотображение
// @match        https://myshows.me/*
// @match        https://coldfilm.ink/*
// @grant        GM.xmlHttpRequest
// @grant        GM.setValue
// @grant        GM.getValue
// @grant        GM_deleteValue
// @grant        GM_addStyle
// @require      https://ajax.googleapis.com/ajax/libs/jquery/3.6.0/jquery.min.js
// @connect      coldfilm.ink
// ==/UserScript==

(async function() {
    const STORAGE_KEY = 'episode_links_auto';
	const ICONS = {
		renewImg: 'data:image/png;base64,',
		download: 'data:image/png;base64,',
		torrent: 'data:image/png;base64,',
		notfound: 'data:image/png;base64,',
		loading: 'data:image/svg+xml;base64,'
	};
    function hasCloudflareCaptcha(doc) {
        return doc.querySelector('form#challenge-form, .cf-error-code, .cf-challenge') !== null;
    }

    function normalizeTitle(title) {
        return title.trim()
            .replace(/^Звездный$/, 'Звёздный')
            .replace(/^Детство Шелдона$/, 'Молодой Шелдон')
            .replace(/^Студия Marvel: Легенды$/, 'Marvel Studios: Легенды')
            .replace(/\s{2,}/g, ' ');
    }

    function tryBetaRequest(episodeId, { name, season, episode }, onSuccess, onFail) {
    let attempts = 0;

    function makeRequest() {
        attempts++;
        console.log('Starting attempt', attempts);

        GM.xmlHttpRequest({
            method: 'GET',
            url: 'https://coldfilm.ink/news/?page' + (attempts - 1),
            onload: res => {
                const doc = new DOMParser().parseFromString(res.responseText, 'text/html');
                if (hasCloudflareCaptcha(doc)) {
                    onFail && onFail('Обнаружена защита Cloudflare');
                    return;
                }

                const expectedTitle = `${name} ${season} сезон ${episode} серия`.toLowerCase();
                const link = [...doc.querySelectorAll('a.kino-h')].find(a => {
                    return (a.getAttribute('title') || '').toLowerCase().includes(expectedTitle);
                });

                if (link) {
                    const href = link.getAttribute('href');
                    fetchTorrent(href, episodeId, onSuccess);
                } else if (attempts < 5) {
                    makeRequest(); // Повторить, если попытки остались
                } else {
                    onFail && onFail('Серия не найдена после 5 страниц');
                }
            },
            onerror: err => {
                if (attempts < 5) {
                    makeRequest(); // Повторить при ошибке, если попытки остались
                } else {
                    onFail && onFail('Ошибка запроса после 5 попыток');
                }
            }
        });
    }

    makeRequest(); // Начать первый запрос
}

    function fetchTorrent(href, episodeId, callback) {
        GM.xmlHttpRequest({
            method: 'GET',
            url: `https://coldfilm.ink${href}`,
            onload: async res => {
                const doc = new DOMParser().parseFromString(res.responseText, 'text/html');
                const torrent = doc.querySelector('.player-box a[href*=".torrent"]')?.getAttribute('href');
                const title = doc.querySelector('h1.kino-h')?.textContent.trim();
                if (torrent && title) {
                    const fullLink = `https://coldfilm.ink${torrent}`;
                    const saved = JSON.parse(await GM.getValue(STORAGE_KEY, '{}'));
                    saved[episodeId] = { link: fullLink, name: title, pagelink:`https://coldfilm.ink${href}`, timestamp: Date.now() };
                    await GM.setValue(STORAGE_KEY, JSON.stringify(saved));
                    callback && callback({ link: fullLink, name: title });
                } else {
                    callback && callback(null);
                }
            },
            onerror: () => callback && callback(null)
        });
    }

    async function getSavedLinks() {
        try {
            return JSON.parse(await GM.getValue(STORAGE_KEY, '{}'));
        } catch (e) {
            return {};
        }
    }

    function addPanelStyles() {
        GM_addStyle(`
            #floatingPanel {
                position: fixed;
                top: 80px;
                left: 0;
                width: 300px;
                max-height: 80vh;
                overflow: auto;
                background: #fff;
                border: 1px solid #ccc;
                z-index: 10000;
                font-size: 13px;
                box-shadow: 0 0 10px rgba(0,0,0,0.2);
            }
            #floatingHeader {
                background: #f0f0f0;
                padding: 8px;
                font-weight: bold;
                text-align: center;
            }
            #episodesTable th, #episodesTable td {
                border: 1px solid #ccc;
                padding: 4px;
            }
            .check-btn {
                padding: 2px 6px;
                cursor: pointer;
            }
			#nameblock{
	  border: 1px solid #bbbbbb;
      border-radius: 5px 5px 0 0;
      background: #039bf31f;
	  border-bottom: 0;
	  display:none;
	  }
	 #hrefblock{
	  border: 1px solid #bbbbbb;
      border-radius: 5px;
      background: #039bf31f;
	  /*border-top: 0;
      margin-top: -2px;*/
	  padding: 5px 0;
	  }
        `);
    }

    async function createFloatingPanel() {
        const saved = await getSavedLinks();

        const $panel = $(
            `<div id="floatingPanel">
                <div id="floatingHeader">Проверка новых серий на <a href="https://coldfilm.ink" target="_blank">coldfilm.ink</a></div>
                <div style="padding: 8px;">
                    <table id="episodesTable" style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr><th>Сериал</th><th>Состояние</th></tr>
                        </thead>
                        <tbody></tbody>
                    </table>
                </div>
            </div>`
        );

        $('body').append($panel);

        $('.UnwatchedEpisodeItem').each((i, el) => {
            const $el = $(el);
            const container = $el.closest('[id^="s"]');
            const name = normalizeTitle(container.find('.Unwatched-showTitle-title').text());
            const season = container.find('.Unwatched-showSeasonTitle').text().split('Сезон')[1]?.trim();
            const episode = $el.find('.UnwatchedEpisodeItem__index span').text().trim();
            const episodeId = `${container.attr('id')}_${episode}`;
            //const label = `${season}с${episode}э`;
			const label = `${season}_сезон ${episode}_серия`;
            const savedEntry = saved[episodeId];
			const hrefblock = `
					<div id="nameblock">${name}</div>
		            <div id="hrefblock">
                    <a href="${savedEntry ? `${savedEntry.link}` : ''}" target="_blank">
                    <img src="${ICONS.torrent}" style="width:20px;vertical-align:middle;"/> Скачать</a>
                    | <a href="${savedEntry ? `${savedEntry.pagelink}` : ''}" target="_blank" title="${name}">Смотреть</a></div>
			`;
            const row = $(`
                <tr class="row-${episodeId}">
				    <td style="border: 1px solid #ccc; padding: 4px; vertical-align:top; text-align: center;">${name}<br><small>${label}</small><div style="margin-top: 10px;text-align: center;" id="res-${episodeId}">${savedEntry ? `${hrefblock}` : ''}</div></td>
                    <td style="border: 1px solid #ccc; padding: 4px; text-align:center; ">
					    <button class="check-btn" style="padding:2px 6px;">Проверить</button>
		                <button id="delit-${episodeId}" style="padding:2px 6px; color:red" title="Удалить строку">X</button>
                    </td>
                </tr>`);
			 row.find(`#delit-${episodeId}`).on('click', function () {
				$(`.row-${episodeId}`).remove();
			});
            row.find('.check-btn').on('click', function () {
                const $cell = row.find(`#res-${episodeId}`);
                $cell.html(`<img src='${ICONS.loading}' style='height:16px;' />`);
                tryBetaRequest(
                    episodeId,
                    { name, season, episode },
                    result => {
                        if (result && result.link) {
                            $cell.html(`${hrefblock}`);
                        } else {
                            $cell.html('<img src="${ICONS.notfound}" title="Серия ещё не переведена" style="height:20px;"/>');
                        }
                    },
                    msg => {
                        $cell.html(`<img src="${ICONS.notfound}" title="Серия ещё не переведена" style="height:20px;"/> ${msg}. <a href="https://coldfilm.ink/news/?page1" target="_blank">Открыть сайт</a>`);
                    }
                );
            });

            $('#episodesTable tbody').append(row);
        });
    }

    if (location.hostname.includes('coldfilm.ink')) {
        const title = document.querySelector('h1.kino-h')?.textContent;
        const torrent = document.querySelector('.player-box a[href*=".torrent"]')?.getAttribute('href');
        if (title && torrent) {
            const link = `https://coldfilm.ink${torrent}`;
            const episodeId = title.replace(/[^a-z0-9а-яё]+/gi, '_').toLowerCase();
            const saved = JSON.parse(await GM.getValue(STORAGE_KEY, '{}'));
            saved[episodeId] = { link, name: title, timestamp: Date.now() };
            await GM.setValue(STORAGE_KEY, JSON.stringify(saved));
            console.log(`✔️ Серия сохранена вручную: ${title}`);
            window.close();
        }
    }

    if (location.hostname.includes('myshows.me')) {
        $(document).ready(() => {
            addPanelStyles();
            createFloatingPanel();
        });
    }
})();
